...
truthy = frozenset(["yes", "Yes", "y", "Y", "true", "True", "TRUE", "YES", "1", "!0"])
...
quest_reward_types_formatted = re.sub(
    r'[{}",]', "", json.dumps(data["quest_reward_types"], indent=2, sort_keys=True)
)
...
def int_or_default(input_parm):
    payload["message"][input_parm] = check_int(input(), payload["message"][input_parm])
...
def gym_cache():
    print("Do you use file caching or does 'gym name' matter? (y/n)", end="\n> ")
    if input() in truthy:
        if not list_cache():
            return False
        print(
...
def reset_timers_and_encounters():
    ...
    if payload["type"] == "pokemon":
        ...
        payload["message"].update(
            {"last_modified": current_time, "occupied_since": current_time - 9000}
        )
        ...
        payload["message"].update({"stop_id": current_time, "timestamp": current_time})
...
def monster_form(webhook_field, monster_id):
    ...
    for id_ in raw_form_names:
        ...
        for form_id_ in raw_form_names[id_]:
            english_form_names[id_][form_id_] = (
                data["form_names"].get(raw_form_names[id_][form_id_])
...
def stop_info(id_field_name, url_field_name, name_field_name):
    ...
    print("What is the pokestop URL you'd like to show as the image?", end="\n> ")
    payload["message"][url_field_name] = input()
    print("Pokestop name", end="\n> ")
    payload["message"][name_field_name] = input()


webhooks_formatted = re.sub('[{}",]', "", json.dumps(whtypes, indent=2, sort_keys=True))
...
print(
    "What is the URL of where you would like to send the webhook?"
...
if type in ["4", "5"]:
    ...
    payload["message"]["level"] = check_int(input(), payload["message"]["level"])
